...
import SaleItem, { ISaleItem, ISaleItemDocument } from '../models/saleItem.model';
...
export const getSaleItemsFromApi = async () => {
    ...
    const filteredData: ISaleItem[] = data.items.map(
      (item: TStore01Response) => ({
        ...
        location3: STORE_CONSTANTS[item.store_ids[0]!]?.includes('location3') || false,
        location4: STORE_CONSTANTS[item.store_ids[0]!]?.includes('location4') || false,
        location5: STORE_CONSTANTS[item.store_ids[0]!]?.includes('location5') || false,
      })
    );
